...
import ItemFormMyCollection from "./ItemFormMyCollection";
...
export default function MyCollection({ setSelectedLink, link }) {
  ...
  const { comicBooks, dispatch } = useMyCollectionContext();
  ...
  useEffect(() => {
    const fetchComicBooks = async () => {
      ...
      if (response.ok) {
        dispatch({ type: "SET_MYCOLLECTION", payload: json });
      }
    };
    fetchComicBooks();
  }, [dispatch]);
  ...
  const handleDelete = async () => {
    selectedRows.map(async (row) => {
      ...
      if (response.ok) {
        dispatch({ type: "DELETE_MYCOLLECTION", payload: json });
      }
    });
  };
